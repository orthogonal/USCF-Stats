class DeltasController < ApplicationController
  ...
  def buildchart
    history = UscfWebsite.get_rating_history_from_id(14433377, 0)
    @data = Array.new
    puts "HISTORY: #{history}"
    history.each do |event|
      result = UscfWebsite.get_results_against_opponents_from_tournament(14433377, event[:id], event[:section])
      result.each do |row|
